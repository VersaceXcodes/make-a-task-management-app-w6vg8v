...
const UV_SignUp: React.FC = () => {
  ...
  const signupMutation = useMutation<AuthResponse, AxiosError<{ error: string }>, SignupRequest>({
    mutationFn: async (newUserData) => {
      const response = await axios.post<AuthResponse>(
        `${api_base_url}/auth/signup`,
        {
          ...
          full_name: undefined, // No full_name input per requirements
        },
        {
    ...
    onSuccess: (data) => {
      ...
      set_auth({ token: data.token, is_authenticated: true, user_id: data.user_profile.user_id });
    ...
    onError: (error) => {
      ...
      if (error.response?.status === 400 && error.response.data?.error) {
        set_validation_errors({ email: error.response.data.error });
      } else {
              ...
              <div>
                ...
                <div className="mt-1">
                  <input
                    ...
                    aria-describedby={
                      validationErrors.passwordConfirmation ? "passwordConfirmation-error" : undefined
                    }
                  />
                </div>
