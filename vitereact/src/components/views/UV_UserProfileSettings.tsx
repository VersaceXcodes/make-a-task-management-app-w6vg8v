...
const UV_UserProfileSettings: React.FC = () => {
  ...
  const userProfile = use_app_store((state) => state.user_profile);
  ...
  const [localSettings, setLocalSettings] = useState<UserSetting>({
  ...
  const logoutMutation = useMutation<LogoutResponse, Error, void>(
    ...
    },
    {
      onSuccess: () => {
        ...
        // Clear user profile and settings global state on logout
  ...
  // Effect: sync global userProfile.full_name changes to local input
  useEffect(() => {
    setLocalFullName(userProfile.full_name ?? "");
  }, [userProfile.full_name]);

  // Effect: sync global userSettings to local state
  useEffect(() => {
  ...
  // No backend password change endpoint specified, so submission will just simulate
  const handleSubmitPasswordChange = (e: React.FormEvent) => {
    ...
    // For MVP: Show a message that password change is not implemented
          ...
          >
            <div>
              ...
              <input
                ...
                readOnly
                className="mt-1 block w-full rounded-md border border-gray-300 bg-gray-100 dark:bg-gray-700 dark:text-gray-100 cursor-not-allowed px-3 py-2"
              />
            </div>
            <div>
              <label
